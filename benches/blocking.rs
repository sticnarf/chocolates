...
mod blocking {
    ...
    pub fn cpu_bound(b: &mut criterion::Bencher) {
...
fn bench_cpu_bound(c: &mut Criterion) {
    c.bench(
        "cpu_bound",
        ParameterizedBenchmark::new("blocking", |b, _| blocking::cpu_bound(b), &[()])
            .with_function("message_passing", |b, _| message_passing::cpu_bound(b)),
    );
}

criterion_group!(benches, bench_cpu_bound);
